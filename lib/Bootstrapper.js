  ...
  */
	_apply(nodes, cb = null) {
		...
		for (let i = 0; i < nodes.length; ++i) {
			...
			resolveQueue.push(this.__container__.resolve(componentName, this).then(registration => {
				...
				this.__container__.factory.inject(registration.component, props, Habitat.create(ele, this.__container__.id, options));
			}).catch(err => {
		...
		Promise.all(resolveQueue.map(p => p.catch(e => e))).then(() => {
			_callback(cb);
		}).catch(err => {
  ...
  */
	setContainer(container, cb = null) {
		if (this.__container__ !== null) {
			Logger.error('RHW02', 'A container is already set. ' + 'Please call dispose() before assigning a new one.');
			return;
		}
