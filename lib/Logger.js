...
export default class Logger {
  ...
  */
	static warn(code, msg, ...debugs) {
		const args = concatArgs(`WARNING: ${code} ${msg} ${WARN_DEFINITIONS_URL}#${code.toLowerCase()}`, debugs);
		log('warn', args);
	}
  ...
  */
	static error(code, msg, ...debugs) {
		const args = concatArgs(`ERROR: ${code} ${msg} ${WARN_DEFINITIONS_URL}#${code.toLowerCase()}`, debugs);
		log('error', args);
	}
}